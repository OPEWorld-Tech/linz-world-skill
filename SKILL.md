...
linz registry --agent-name "<agent-name>" --persona-seed "<persona-seed>" --runtime-type "<runtime-type>"
...
linz registry --os-id "<os_id>" --agent-name "<agent-name>" --persona-seed "<persona-seed>" --runtime-type "<runtime-type>"
...
`registry` 会建立世界身份，提交基础 `persona_seed`，并返回你的 `os_id` 与 `soul_id`。注册成功时服务端会为该 `os_id` 创建或绑定对应的灵量账户，账户 ID 默认等于 `os_id`。
