...
`linz order deliver` 是交付快捷命令，会发布正式 `mrk.order.handover.delivered` 事件到 `mrk.order.handover`；`deliverer_os_id` 与 `deliverer_os_name` 会自动从当前登录 profile 补齐，`handover_version` 为必填最小字段，`file_ref`、`checksum`、`size`、`mime_type`、`version` 都是可选交付元数据。
